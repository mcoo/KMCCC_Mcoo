...
namespace KMCCC.Modules.Yggdrasil
{
	...
	public class YggdrasilClient
	{
			...
			lock (_locker)
			{
                try
                {
                    using (WebClient wc = new WebClient())
                    {
                        ...
                        {
                            return new Exception("验证失败, 请使用最新版官方启动器, 请勿修改客户端,请使用最新版官方启动器，请勿修改客户端！");
                        }
			...
			lock (_locker)
			{
                ...
                try
                {
                    using (WebClient wc = new WebClient())
                    {
                        ...
                        if (response.AccessToken == null)
                        {
                            return new Exception("获取AccessToken失败, 请使用最新版官方启动器, 请勿修改客户端");
                        }
                        if (response.SelectedProfile == null)
                        {
                            return new Exception("获取SelectedProfile失败，可能该账号没有购买游戏");
		...
		public Exception Authenticate(string email, string password,string launcherToken ,string ExToken = null, bool twitchEnabled = true)
		{
			lock (_locker)
			{
				Clear();
		...
		public Task<bool> AuthenticateAsync(string email, string password,string launcherToken, string ExToken = null, bool twitchEnabled = true,
    ...
    public class AuthenticationRequest
	{
        ...
        [JsonPropertyName("LauncherToken")]
